// Simple NCCL in-node in-place all-reduce benchmark on 2 GPUs (0 and 1).
// All-Reduce: each rank has input data, all ranks get the same result (sum of all inputs).
// In-place: input and output use the same buffer.
...
void* run_rank(void* arg) {
  ...
  // For all-reduce, each rank has count_per_rank elements
  size_t bytes_per_iter = count_per_rank * sizeof(float);

  float* d_buf = nullptr;
  CHECK_CUDA(cudaMalloc(&d_buf, bytes_per_iter));

  // Initialize buffer with rank id as value (using same kernel signature as all-gather)
  int threads = 256;
  int blocks = static_cast<int>((count_per_rank + threads - 1) / threads);
  init_buffer<<<blocks, threads, 0, stream>>>(d_buf, 0, count_per_rank,
  ...
  // Average latency per all-reduce (ms) for this GPU
  ...
  printf("[Rank %d] count_per_rank=%zu, avg_latency=%.3f ms, throughput=%.3f GB/s\n",
         rank, count_per_rank, avg_ms, input_gbps);
  fflush(stdout);
