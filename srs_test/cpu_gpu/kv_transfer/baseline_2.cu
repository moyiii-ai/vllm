...
int main() {
  ...
  std::cout << "Starting baseline_2 transfers:" << std::endl;
  ...
  std::cout << "  Measurement: Single measurement after all iterations" << std::endl;
  std::cout << std::endl;

  CUDA_CHECK(cudaSetDevice(GPU2_ID));

  // Record start events before all iterations
  // CUDA_CHECK(cudaSetDevice(GPU1_ID));
  ...
  // Perform all transfers without timing in the loop
  ...
  // Record stop events after all iterations
  // CUDA_CHECK(cudaSetDevice(GPU1_ID));
  ...
  // Calculate total time for each transfer
  float total_time_gpu2_to_gpu1 = 0.0f;
  float total_time_gpu2_to_gpu0 = 0.0f;
  float total_time_gpu2_to_cpu = 0.0f;

  CUDA_CHECK(cudaEventElapsedTime(&total_time_gpu2_to_gpu1, start_gpu2_to_gpu1, stop_gpu2_to_gpu1));
  CUDA_CHECK(cudaEventElapsedTime(&total_time_gpu2_to_gpu0, start_gpu2_to_gpu0, stop_gpu2_to_gpu0));
  CUDA_CHECK(cudaEventElapsedTime(&total_time_gpu2_to_cpu, start_gpu2_to_cpu, stop_gpu2_to_cpu));
