...
static const size_t DATA_SIZE =  (32ULL * 1024ULL * 1024ULL);
...
static int client_remote_memory_ops()
{
    struct ibv_wc wc;
    int ret = -1;
    size_t transfer_size = DATA_SIZE;
    ...
    client_send_sge.length = transfer_size;
    client_send_sge.lkey = client_src_mr->lkey;

    
    while (!stop_loop) {
        if(total_bytes%(4*1024ULL*1024ULL*1024ULL)==0){
            total_bytes = 0;
        ...
        client_send_wr.sg_list = &client_send_sge;
        client_send_wr.num_sge = 1;
        client_send_wr.opcode = operation_type? IBV_WR_RDMA_WRITE : IBV_WR_RDMA_READ;
        client_send_wr.send_flags = IBV_SEND_SIGNALED;
        ...
        if(total_bytes%(4*1024ULL*1024ULL*1024ULL)==0){
            ...
            double sample_timestamp = (end_time.tv_sec + start_time.tv_sec) / 2.0 + 
                                        (end_time.tv_nsec + start_time.tv_nsec) / 2.0 / 1e9;
            double throughput = total_bytes / elapsed_time / (1024.0 * 1024.0 * 1024.0);
            
            fprintf(log_file, "%.6f\t%.2f\n", 
                            sample_timestamp,throughput);
...
int main(int argc, char **argv)
{
    ...
    while ((option = getopt(argc, argv, "a:p:rwl:")) != -1) {
        switch (option) {
            case 'a':
