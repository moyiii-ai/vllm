#!/bin/bash
# RDMA write server script (ib_write_bw).
# Default: h2, numa0, CPU only. Override via env: NETNS, NUMA_NODE, USE_GPU (1=GPU, 0=CPU), GPU_ID.

# Configuration (env overrides supported)
NUMA_NODE=${NUMA_NODE:-0}
NETNS=${NETNS:-h2}
# USE_GPU: 1 = bind to GPU (use GPU_ID), 0 or unset = CPU only
USE_GPU=${USE_GPU:-0}
GPU_ID=${GPU_ID:-4}
IB_DEVICE=${IB_DEVICE:-mlx5_0}
IB_PORT=${IB_PORT:-1}
GID_INDEX=${GID_INDEX:-0}
# TCP port for control connection (server listens); use different ports for read vs write to avoid conflict
PORT=${PORT:-18515}
...
# Run ib_write_bw server in netns
echo "Starting ib_write_bw server in netns $NETNS..."
