#!/bin/bash
# Start one vLLM worker for the MultiConnector Flow experiment.
#
#   local  (Node A): MultiConnector[OffloadingConnector(kv_both),
#                                   MooncakeStoreConnector(kv_consumer)]
#                    Mooncake mode=standalone-store (no DRAM mount)
#                    Offloading CPU budget = OFFLOADING_CPU_BYTES (default 128GiB)
#
#   remote (Node B): MooncakeStoreConnector(kv_both), embedded GLOBAL_SEGMENT_SIZE
#                    Put pinned to H2_SEGMENT by default
#
# Usage:
#   ./start_server.sh local
#   ./start_server.sh remote
#   ./start_server.sh --l40s-l40s local
#   ./start_server.sh --no-prefer-segment remote   # B: disable Put pin
...
# Preferred-segment pin applies to Node B only (the sole Mooncake producer).
...
write_mooncake_config "$CFG" "$RDMA" "$ROLE"
...
KV_TRANSFER_CONFIG="$(kv_transfer_config_json "$ROLE" "$CROSS_LAYERS_JSON")"
...
if [[ "$ROLE" == "local" ]]; then
  echo "  Node A: MultiConnector[Offloading + MooncakeStore(kv_consumer)]"
  echo "  Offloading CPU: $OFFLOADING_CPU_BYTES bytes  block_size=$OFFLOADING_BLOCK_SIZE"
  echo "  Mooncake: standalone-store (no DRAM mount; Get-only)"
  warn_if_local_gpu_pcie_slow
else
  echo "  Node B: MooncakeStoreConnector(kv_both) embedded segment=$GLOBAL_SEGMENT_SIZE"
...
# Node A never Puts to Mooncake — never set preferred segment.
# Node B defaults to pinning Puts to its own segment.
if [[ "$ROLE" == "remote" && "$PREFER_SEGMENT" == "1" ]]; then
...
# Escape single quotes in JSON for embedding inside bash -c '...'.
