...
case "${SKIP_REPORT:-0}" in
  1|true|yes|on) ;;
  *)
    if [[ -f "$SCRIPT_DIR/report_server_stats.sh" ]]; then
      echo
      bash "$SCRIPT_DIR/report_server_stats.sh" || true
    elif [[ -f "$MOONCAKE_TEST_DIR/report_server_stats.sh" ]]; then
      echo
      # Reuse mooncake reporter against this LOG_DIR.
      LOG_DIR="$LOG_DIR" bash "$MOONCAKE_TEST_DIR/report_server_stats.sh" || true
    fi
    ;;
esac
