...
#   ./start_all.sh --no-prefer-segment   # Node B: disable Put pin
set -euo pipefail
...
echo "=== GPU_PAIR=$GPU_PAIR (MultiConnector Flow) ==="
echo "  Node A local:  GPU$LOCAL_GPU ($LOCAL_GPU_NAME) Offloading ${OFFLOADING_CPU_BYTES}B + Mooncake consumer"
echo "  Node B remote: GPU$REMOTE_GPU ($REMOTE_GPU_NAME) Mooncake embedded $GLOBAL_SEGMENT_SIZE"
echo "  PREFER_SEGMENT(B)=$PREFER_SEGMENT"
...
echo
echo "Stack ready (GPU_PAIR=$GPU_PAIR, PREFER_SEGMENT(B)=$PREFER_SEGMENT):"
echo "  master:  $MASTER_IP:$MASTER_PORT (NUMA$MASTER_NUMA)"
echo "  Node A:  http://127.0.0.1:$VLLM_PORT_LOCAL  ($H1_NS, GPU$LOCAL_GPU $LOCAL_GPU_NAME)"
echo "  Node B:  http://127.0.0.1:$VLLM_PORT_REMOTE ($H2_NS, GPU$REMOTE_GPU $REMOTE_GPU_NAME)"
