...
int main(int argc, char* argv[]) {
  ...
  uint32_t *d_src0 = nullptr, *d_src1 = nullptr, *d_dst0 = nullptr,
           *d_dst1 = nullptr;
  ...
  for (size_t si = 0; si < data_sizes.size(); ++si) {
    ...
    for (int iter = 0; iter < repeat; ++iter) {
      if (mode == 1) {
        ...
        if (op == "read") {
          if (base_gpu == 0)
            peerReadKernelV4<<<GRID_SIZE, BLOCK_SIZE>>>(d_dst0, d_src1, n_vec4,
                                                        d_checksum0);
          else
            peerReadKernelV4<<<GRID_SIZE, BLOCK_SIZE>>>(d_dst1, d_src0, n_vec4,
                                                        d_checksum1);
        } else {
          if (base_gpu == 0)
            peerWriteKernelV4<<<GRID_SIZE, BLOCK_SIZE>>>(d_dst1, d_src0, n_vec4,
                                                         d_checksum0);
          else
            peerWriteKernelV4<<<GRID_SIZE, BLOCK_SIZE>>>(d_dst0, d_src1, n_vec4,
                                                         d_checksum1);
        }
        ...
        if (op == "read")
          peerReadKernelV4<<<GRID_SIZE, BLOCK_SIZE, 0, stream0>>>(
              d_dst0, d_src1, n_vec4, d_checksum0);
        else
          peerWriteKernelV4<<<GRID_SIZE, BLOCK_SIZE, 0, stream0>>>(
              d_dst1, d_src0, n_vec4, d_checksum1);
        cudaEventRecord(stop0, stream0);

        checkCuda(cudaSetDevice(1));
        checkCuda(cudaEventRecord(start1, stream1));
        if (op == "read")
          peerReadKernelV4<<<GRID_SIZE, BLOCK_SIZE, 0, stream1>>>(
              d_dst1, d_src0, n_vec4, d_checksum1);
        else
          peerWriteKernelV4<<<GRID_SIZE, BLOCK_SIZE, 0, stream1>>>(
              d_dst0, d_src1, n_vec4, d_checksum0);
  ...
  cudaFree(d_src0);
  cudaFree(d_dst0);
  cudaFree(d_checksum0);

  checkCuda(cudaSetDevice(1));
  cudaFree(d_src1);
  cudaFree(d_dst1);
  cudaFree(d_checksum1);
