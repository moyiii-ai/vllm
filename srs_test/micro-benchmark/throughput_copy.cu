  ...
  for (int si = 0; si < data_sizes.size(); si++) {
    ...
    for (int i = 0; i < repeat; i++) {
      if (mode == 1) {
        ...
        if (op == "read") {
          if (dev == 0)
            copyKernel<<<GRID_SIZE, BLOCK_SIZE, 0, stream>>>(d_dst0, d_src1,
                                                             numElements);
          else
            copyKernel<<<GRID_SIZE, BLOCK_SIZE, 0, stream>>>(d_dst1, d_src0,
                                                             numElements);
        } else {
          if (dev == 0)
            copyKernel<<<GRID_SIZE, BLOCK_SIZE, 0, stream>>>(d_dst1, d_src0,
        ...
        checkCuda(cudaSetDevice(0));
        if (op == "read")
          copyKernel<<<GRID_SIZE, BLOCK_SIZE, 0, stream0>>>(d_dst0, d_src1,
                                                            numElements);
        else
          copyKernel<<<GRID_SIZE, BLOCK_SIZE, 0, stream0>>>(d_dst1, d_src0,
                                                            numElements);

        checkCuda(cudaSetDevice(1));
        if (op == "read")
          copyKernel<<<GRID_SIZE, BLOCK_SIZE, 0, stream1>>>(d_dst1, d_src0,
                                                            numElements);
        else
          copyKernel<<<GRID_SIZE, BLOCK_SIZE, 0, stream1>>>(d_dst0, d_src1,
                                                            numElements);
