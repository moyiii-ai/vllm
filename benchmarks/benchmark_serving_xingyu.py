...
def sample_longbench_requests(
    ...
    with open(dataset_path, "r", encoding="utf-8") as file:
        for line in file:
            ...
            prompt_len = len(tokenizer(data["prompt"]).input_ids)
            # filtered_dataset.append((data["prompt"], prompt_len, data["max_len"], None))
            filtered_dataset.append((data["prompt"], prompt_len, output_len, None))

    if warmup:
        # Take only hit_ratio of the requests
        filtered_dataset = filtered_dataset[: int(len(filtered_dataset) * hit_ratio)]
    else:
        filtered_dataset = random.choices(filtered_dataset, k=num_requests)
...
async def benchmark(
    ...
    test_input = RequestFuncInput(
        ...
        logprobs=logprobs,
        # New version of request_func not support best_of.
        # best_of=best_of,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="sharegpt",
        choices=["longbench", "sharegpt", "burstgpt", "sonnet", "random", "hf"],
    ...
    parser.add_argument(
        "--warmup",
        action="store_true",
        help="Warmup the storage server with the prompts",
    )
    ...
    parser.add_argument(
        ...
        default=1.0,
        metavar="0.0-1.0",
        help="Cache hit ratio for the warmup",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Output length for longbench. Should enable ignore_eos to take effect.",
    )
